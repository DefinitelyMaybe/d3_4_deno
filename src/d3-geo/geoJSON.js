...
(function (GeoJSON) {
  ...
  function InvalidGeometryError() {
    ...
    this.message = this.message ||
      "Invalid Geometry: " + "item: " + JSON.stringify(item) + ", params: " +
    ...
    if (Array.isArray(objects)) {
      ...
      objects.forEach(function (item) {
        var feature = getFeature(
          { item: item, params: settings, propFunc: propFunc },
        );
        if (
          settings.removeInvalidGeometries !== true ||
          GeoJSON.isGeometryValid(feature.geometry)
        ) {
      ...
      geojson = getFeature(
        { item: objects, params: settings, propFunc: propFunc },
      );
      addOptionals(geojson, settings);
    }
  ...
  function buildGeom(item, params) {
    ...
    for (var gtype in params.geom) {
      ...
      */
        ...
        Array.isArray(val) && item.hasOwnProperty(val[0]) &&
        item.hasOwnProperty(val[1]) && item.hasOwnProperty(val[2])
      ) {
        geom = {
          type: gtype,
          coordinates: [
        ...
        Array.isArray(val) && isNested(val[0]) && isNested(val[1]) &&
        isNested(val[2])
      ) {
        geom = undefined;
        ...
        Array.isArray(val) && val[0].constructor.name === "Object" &&
    ...
    if (
      params.doThrows && params.doThrows.invalidGeometry &&
  ...
  function getPropFunction(params) {
    ...
    if (!params.exclude && !params.include) {
      ...
      func = function (properties) {
        for (var attr in this) {
          if (
            this.hasOwnProperty(attr) && (geomAttrs.indexOf(attr) === -1) &&
