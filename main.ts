  ...
  return fetch(srcURL)
    .then((res) => {
      if (res.status == 200) {
        ...
        console.log(path, moduleName, relativePath);
      }
    })
    .then((text) => {
      if (text) {
...
function adjustscriptURLS(path: string) {
  ...
  src = src.replaceAll(/import .+? from ("|')d3-.+?("|')/g, (m) => {
    if (!m.includes("mod.js")) {
      ...
      if (m.includes("'")) {
        let newValue = m.replace(
          /'d3-/g,
          `'${"../".repeat(scriptDepth - 2)}d3-`,
        );
        newValue = path.endsWith(".js")
          ? newValue.replace(/'$/g, "/mod.js'")
          : newValue.replace(/'$/g, "/mod.d.ts'");
        return newValue;
      } else {
        let newValue = m.replace(
          /"d3-/g,
...
// gather initial index.js files and rename them to mod.js files
const initIndices = [];
for (let index = 0; index < d3modules.length; index++) {
  const moduleName = d3modules[index];
  initIndices.push(initIndex(moduleName));
}
...
const geoJsonURL =
  `https://raw.githubusercontent.com/caseycesari/GeoJSON.js/master/geojson.js`;
const geoJsonTypesURL =
  `https://raw.githubusercontent.com/DefinitelyTyped/DefinitelyTyped/master/types/geojson/index.d.ts`;
const delaunayURL =
  `https://raw.githubusercontent.com/mapbox/delaunator/master/index.js`;
const delaunayTypesURL =
  `https://raw.githubusercontent.com/DefinitelyTyped/DefinitelyTyped/master/types/d3-delaunay/index.d.ts`;
