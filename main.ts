...
function populateFromSource(src:string, modName:string) {
  ...
  for (const match of matches) {
    ...
    // construct the url
    const srcURL = `https://raw.githubusercontent.com/d3/${modName}/master/src/${scriptRelativePath}`
  ...
  .then((res) => {
    if (res.status == 200) {
      ...
      .then((text) => {
        ...
        // add deno ref to types and dom
        text = text.replace(/^/, `/// <reference types="./${relativePathTS}" />\n/// <reference lib="dom" />\n`)
...
// for good measure we'll run deno fmt across the library
Deno.run({
  cmd: ["deno", "fmt", d3Dir],
})